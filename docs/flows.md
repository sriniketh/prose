...
ViewHighlightsScreen → hiltViewModel() populates ViewHighlightsViewModel's SavedStateHandle with bookId
  → init collects HighlightsRepository.getAllHighlightsForBookFromDb(bookId) once, for the ViewModel's lifetime
...
capture_and_crop_image/{bookId}
  → CaptureAndCropImageScreen + CaptureAndCropImageViewModel
      imageUri = savedStateHandle["imageUri"] ?: FileSource.createNewFile("<uuid>.jpg").also { savedStateHandle["imageUri"] = it }
